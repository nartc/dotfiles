---
name: nx-typecheck-invoker
description: Use this agent when you need to run TypeScript type checking on projects within an Nx monorepo workspace. This agent handles invoking the correct nx typecheck command, automatically detecting the package manager (npm, pnpm, yarn), resolving project names and target names when they don't exist, and providing clear feedback on type check results with error analysis.\n\nExamples:\n\n<example>\nContext: User wants to verify types after implementing a new feature in an Nx workspace.\nuser: "I just finished implementing the user authentication feature in the auth-lib project. Can you check if there are any type errors?"\nassistant: "I'll use the nx-typecheck-invoker agent to run type checking on your auth-lib project."\n<Task tool invocation to launch nx-typecheck-invoker with project: auth-lib, target: typecheck>\n</example>\n\n<example>\nContext: User completed code changes and wants to validate types before committing.\nuser: "Please run typecheck on my-app"\nassistant: "I'll invoke the nx-typecheck-invoker agent to run the typecheck target on my-app."\n<Task tool invocation to launch nx-typecheck-invoker with project: my-app, target: typecheck>\n</example>\n\n<example>\nContext: User is unsure of the exact project name in their Nx workspace.\nuser: "Can you typecheck the frontend dashboard? I think it's called dashboard-ui or something like that."\nassistant: "I'll use the nx-typecheck-invoker agent to find the correct project name and run type checking on it."\n<Task tool invocation to launch nx-typecheck-invoker with project: dashboard-ui, target: typecheck>\n</example>\n\n<example>\nContext: After writing code, proactively checking types before marking task complete.\nassistant: "Now that I've implemented the API endpoints, let me run type checking to ensure there are no type errors."\n<Task tool invocation to launch nx-typecheck-invoker with project: api-service, target: typecheck>\n</example>
model: sonnet
color: blue
---
